...
static void ResolveAxis(float *pos, float *vel, float other, float w, float h, bool vertical) {
  float remaining = *vel * GetFrameTime(); if (remaining == 0.0f) return; float sign = (remaining > 0) ? 1.0f : -1.0f;
  while (remaining != 0.0f) {
    float step = remaining; float maxStep = (float)SQUARE_SIZE - 1.0f; if (step > maxStep) step = maxStep; if (step < -maxStep) step = -maxStep;
    float newPos = *pos + step; float x = vertical ? other : newPos; float y = vertical ? newPos : other;
    if (AABBOverlapsSolid(x, y, w, h)) {
      int pixels = (int)fabsf(step);
      for (int i = 0; i < pixels; ++i) {
        newPos = *pos + sign; x = vertical ? other : newPos; y = vertical ? newPos : other;
        if (AABBOverlapsSolid(x, y, w, h)) { *vel = 0.0f; return; }
        *pos = newPos;
      }
    } else { *pos = newPos; }
    remaining -= step;
...
static bool PlayerTouchesHazard(const GameState *g) {
  ...
  for (int y=0;y<GRID_ROWS;++y)
    for (int x=0;x<GRID_COLS;++x) {
      TileType t = editor.tiles[y][x]; if (!IsHazardTile(t)) continue; Rectangle lr = LaserStripeRect((Vector2){CellToWorld(x), CellToWorld(y)});
...
void UpdateGame(GameState *game) {
  ...
  float dt = GetFrameTime(); if (dt > 0.033f) dt = 0.033f; game->runTime += dt; bool didGroundJumpThisFrame = false;
  if (game->coyoteTimer > 0.0f) game->coyoteTimer -= dt; if (game->jumpBufferTimer > 0.0f) game->jumpBufferTimer -= dt; if (game->groundStickTimer > 0.0f) game->groundStickTimer -= dt;

  bool wantJumpPress = InputPressed(ACT_JUMP);
  bool left = InputDown(ACT_LEFT); bool right = InputDown(ACT_RIGHT); bool down = InputDown(ACT_DOWN);
  ...
  if (left && !right) game->playerVel.x -= accel * dt;
  else if (right && !left) game->playerVel.x += accel * dt;
  else if (game->onGround) game->playerVel.x *= game->crouching ? CROUCH_FRICTION : GROUND_FRICTION;
  else game->playerVel.x *= AIR_FRICTION;

  if (game->playerVel.x > maxSpeedX) game->playerVel.x = maxSpeedX; if (game->playerVel.x < -maxSpeedX) game->playerVel.x = -maxSpeedX;
  game->playerVel.y += GRAVITY * dt; if (game->playerVel.y > MAX_SPEED_Y) game->playerVel.y = MAX_SPEED_Y;
  ...
  if (game->jumpBufferTimer > 0.0f && canJumpNow) {
    game->playerVel.y = JUMP_SPEED; game->onGround = false; game->coyoteTimer = 0.0f; game->jumpBufferTimer = 0.0f; didGroundJumpThisFrame = true;
  }
  ...
  if (game->jumpBufferTimer > 0.0f && !game->onGround && (touchingLeft || touchingRight)) {
    ...
    game->jumpBufferTimer = 0.0f; game->coyoteTimer = 0.0f;
  }

  float newX = game->playerPos.x; float newY = game->playerPos.y;
  ResolveAxis(&newX, &game->playerVel.x, newY, PLAYER_W, aabbH, false);
  ResolveAxis(&newY, &game->playerVel.y, newX, PLAYER_W, aabbH, true);
  bool wasGround = game->onGround; game->onGround = false;

  float belowY = newY + aabbH + 1.0f; int leftCell = WorldToCellX(newX + 1.0f); int rightCell = WorldToCellX(newX + PLAYER_W - 2.0f); int belowCellY = WorldToCellY(belowY);
  for (int cx = leftCell; cx <= rightCell; ++cx) if (BlockAtCell(cx, belowCellY)) { game->onGround = true; break; }
  ...
  if (newX < 0) { newX = 0; game->playerVel.x = 0; }
  if (newX > WINDOW_WIDTH - PLAYER_W) { newX = WINDOW_WIDTH - PLAYER_W; game->playerVel.x = 0; }
  if (newY < 0) { newY = 0; game->playerVel.y = 0; }
  if (newY > WINDOW_HEIGHT - aabbH) { newY = WINDOW_HEIGHT - aabbH; game->playerVel.y = 0; game->onGround = true; }

  game->playerPos.x = newX; game->playerPos.y = newY; if (game->onGround) game->coyoteTimer = COYOTE_TIME;

  if (CheckCollisionRecs(PlayerAABB(game), ExitAABB(game))) {
    victory = true; game->score = (int)(game->runTime * 1000.0f); Audio_PlayVictory();
  }
  if (PlayerTouchesHazard(game)) { death = true; Audio_PlayDeath(); }
}
...
void Game_ClearOutcome(void) { victory = false; death = false; }
