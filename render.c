...
void Render_Init(void) {
    if (gIdleTex.id == 0) gIdleTex = LoadTexture("assets/GlideManIdle.png");
    if (gRunTex.id == 0) gRunTex = LoadTexture("assets/GlideManRun.png");
}

void Render_Deinit(void) {
    if (gIdleTex.id != 0) { UnloadTexture(gIdleTex); gIdleTex.id = 0; }
    if (gRunTex.id != 0) { UnloadTexture(gRunTex); gRunTex.id = 0; }
}
