...
static Rectangle UiListItemRect(const UiListSpec *spec, int index) {
  float x = 20.0f; float y = spec->startY + index * spec->stepY; float w = (float)(GetScreenWidth() - 40); float h = spec->itemHeight;
  return (Rectangle){ x, y, w, h };
}

static int UiListIndexAtMouse(Vector2 m, const UiListSpec *spec, int itemCount) {
  for (int i=0;i<itemCount;++i) if (CheckCollisionPointRec(m, UiListItemRect(spec,i))) return i; return -1;
}
...
bool InputGate_BeginFrameBlocked(void) {
  if (gInputGate == IG_BLOCK_ONCE || gInputGate == IG_LATCHED) {
    ...
    gInputGate = IG_FREE; return false;
  }
  return false;
}
...
void UiListHandle(const UiListSpec *spec, int *selected, int itemCount, bool *outActivated) {
  ...
  if (fabsf(m.x - gUiLastMouse.x) > 1.0f || fabsf(m.y - gUiLastMouse.y) > 1.0f) { gUiSuppressHover = false; gUiLastMouse = m; }
  ...
  if (up) { if (*selected > 0) (*selected)--; else *selected = itemCount>0? itemCount-1:0; }
  if (down) { if (*selected < itemCount-1) (*selected)++; else *selected = 0; }
...
void UiListRenderCB(const UiListSpec *spec, int selected, int itemCount, LabelAtFn labelAt, void *ud,
  ...
  for (int i=0;i<itemCount;++i) {
    Rectangle r = UiListItemRect(spec, i); if (hover == i) { Rectangle hr = r; hr.y -= 2; DrawRectangleRec(hr, (Color){230,230,230,255}); }
    Color c = (selected == i) ? RED : BLUE; const char *label = labelAt ? labelAt(i, ud) : ""; DrawText(label, (int)r.x, (int)r.y, spec->fontSize, c);
  }
...
void RenderMessageScreen(const char *firstText, Color firstColor, int firstFontSize, ...) {
  if (!firstText) return;
  typedef struct { const char *text; Color color; int size; } Line; Line lines[16]; int count = 0; lines[count++] = (Line){ firstText, firstColor, firstFontSize };
  va_list ap; va_start(ap, firstFontSize);
  while (count < (int)(sizeof(lines)/sizeof(lines[0]))) {
    const char *t = va_arg(ap, const char*); if (t == NULL) break; Color c = va_arg(ap, Color); int sz = va_arg(ap, int); lines[count++] = (Line){ t, c, sz };
  }
  va_end(ap);
  const int spacing = 10; int totalH = 0; for (int i=0;i<count;++i) totalH += lines[i].size; if (count>1) totalH += spacing*(count-1);
  int cx = GetScreenWidth()/2; int y = GetScreenHeight()/2 - totalH/2;
  for (int i=0;i<count;++i) { int w = MeasureText(lines[i].text, lines[i].size); DrawText(lines[i].text, cx - w/2, y, lines[i].size, lines[i].color); y += lines[i].size + spacing; }
}

void RenderVictory(const struct GameState *game) {
  float seconds = (float)game->score / 1000.0f; const char *scoreTxt = TextFormat("Score: %.2f s", seconds);
  RenderMessageScreen("VICTORY!", GREEN, 40, "You reached the exit.", DARKGRAY, 24, scoreTxt, BLUE, 28, "Enter: restart | Space/Esc: menu", BLUE, 20, NULL);
}
void RenderDeath(void) {
  RenderMessageScreen("YOU DIED!", RED, 40, "You touched a laser.", DARKGRAY, 24, "Enter: restart | Space/Esc: menu", BLUE, 20, NULL);
}
