...
Vector2 SnapToGrid(Vector2 p) {
  ...
  if (gx < 0) gx = 0; if (gx > WINDOW_WIDTH - SQUARE_SIZE) gx = WINDOW_WIDTH - SQUARE_SIZE;
  if (gy < 0) gy = 0; if (gy > WINDOW_HEIGHT - SQUARE_SIZE) gy = WINDOW_HEIGHT - SQUARE_SIZE;
...
void FillPerimeter(LevelEditorState *ed) {
  for (int x = 0; x < GRID_COLS; ++x) { SetTile(ed, x, 0, TILE_BLOCK); SetTile(ed, x, GRID_ROWS - 1, TILE_BLOCK); }
  for (int y = 1; y < GRID_ROWS - 1; ++y) { SetTile(ed, 0, y, TILE_BLOCK); SetTile(ed, GRID_COLS - 1, y, TILE_BLOCK); }
}
...
bool SaveLevelBinary(const GameState *game, const LevelEditorState *ed) {
  ...
  if (!f) return false;
  const char magic[4] = {'L','V','L','1'}; uint8_t version = 1; uint16_t cols = (uint16_t)GRID_COLS, rows = (uint16_t)GRID_ROWS;
  if (fwrite(magic,1,4,f)!=4) { fclose(f); return false; }
  if (fwrite(&version,1,1,f)!=1) { fclose(f); return false; }
  if (fwrite(&cols,sizeof(cols),1,f)!=1) { fclose(f); return false; }
  if (fwrite(&rows,sizeof(rows),1,f)!=1) { fclose(f); return false; }
  Vector2 p = game->playerPos, e = game->exitPos;
  FindTileWorldPos(ed, TILE_PLAYER, &p); FindTileWorldPos(ed, TILE_EXIT, &e);
  int32_t px=(int32_t)p.x, py=(int32_t)p.y, ex=(int32_t)e.x, ey=(int32_t)e.y;
  if (fwrite(&px,sizeof(px),1,f)!=1) { fclose(f); return false; }
  if (fwrite(&py,sizeof(py),1,f)!=1) { fclose(f); return false; }
  if (fwrite(&ex,sizeof(ex),1,f)!=1) { fclose(f); return false; }
  if (fwrite(&ey,sizeof(ey),1,f)!=1) { fclose(f); return false; }
  for (int y=0;y<GRID_ROWS;++y)
    for (int x=0;x<GRID_COLS;++x) {
      uint8_t t=(uint8_t)ed->tiles[y][x]; if (fwrite(&t,1,1,f)!=1) { fclose(f); return false; }
    }
  fclose(f); return true;
}

bool LoadLevelBinary(GameState *game, LevelEditorState *ed) {
  FILE *f = fopen(gLevelBinPath, "rb"); if (!f) return false;
  char magic[4]; uint8_t version=0; uint16_t cols=0, rows=0;
  if (fread(magic,1,4,f)!=4) { fclose(f); return false; }
  if (memcmp(magic,"LVL1",4)!=0) { fclose(f); return false; }
  if (fread(&version,1,1,f)!=1) { fclose(f); return false; }
  if (version!=1) { fclose(f); return false; }
  if (fread(&cols,sizeof(cols),1,f)!=1) { fclose(f); return false; }
  if (fread(&rows,sizeof(rows),1,f)!=1) { fclose(f); return false; }
  if (cols!=GRID_COLS || rows!=GRID_ROWS) { fclose(f); return false; }
  int32_t px,py,ex,ey;
  ...
  for (int y=0;y<GRID_ROWS;++y)
    for (int x=0;x<GRID_COLS;++x) {
      uint8_t t; if (fread(&t,1,1,f)!=1) { fclose(f); return false; }
...
static int ParseLevelNumber(const char *baseName) {
  if (!baseName) return -1; if (strncmp(baseName,"level",5)!=0) return -1; const char *p = baseName+5; if (!*p) return -1; int n=0;
  while (*p) { if (*p<'0'||*p>'9') return -1; n = n*10 + (*p - '0'); ++p; }
  if (n<=0) return -1; return n-1;
}

static void CatalogSortByNumber(LevelCatalog *cat) {
  for (int i=1;i<cat->count;++i) {
    LevelEntry key=cat->items[i]; int kn=ParseLevelNumber(key.baseName); int j=i-1; 
    while (j>=0) { int jn=ParseLevelNumber(cat->items[j].baseName); if (jn<=kn && !(jn==-1 && kn!=-1)) break; cat->items[j+1]=cat->items[j]; --j; }
    cat->items[j+1]=key;
  }
}
...
void ScanLevels(LevelCatalog *cat) {
  ...
  if (d) {
    ...
    while ((ent = readdir(d)) != NULL) {
      if (ent->d_type == DT_DIR) continue; const char *name = ent->d_name; size_t len = strlen(name);
...
int FindNextLevelIndex(void) {
  LevelCatalog tmp; ScanLevels(&tmp); int maxSeen=-1; for (int i=0;i<tmp.count;++i) { int n=ParseLevelNumber(tmp.items[i].baseName); if (n>maxSeen) maxSeen=n; }
  return maxSeen+1;
}
