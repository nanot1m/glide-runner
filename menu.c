...
static const char *MenuLabelAtCB(int i, void *ud) { (void)ud; return gMenuItems[i]; }

void UpdateMenu(ScreenState *screen, int *selected) {
  int prev = *selected; bool activate = false; UiListHandle(&MENU_SPEC, selected, MENU_OPTION_COUNT, &activate);
  ...
  if (activate) {
    ...
    if (*selected == MENU_EDIT_EXISTING) {
      *screen = SCREEN_SELECT_EDIT;
      ...
      gCreateNewRequested = true; snprintf(gLevelBinPath, sizeof(gLevelBinPath), "%s", LEVEL_FILE_BIN); *screen = SCREEN_LEVEL_EDITOR;
