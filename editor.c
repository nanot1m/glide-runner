...
void UpdateLevelEditor(ScreenState *screen, GameState *game) {
  ...
  double now = GetTime(); bool moved = false;
  if (now - arrowLastTime >= arrowInterval) {
    if (IsKeyDown(KEY_RIGHT)) { editor.cursor.x += SQUARE_SIZE; moved = true; }
    if (IsKeyDown(KEY_LEFT))  { editor.cursor.x -= SQUARE_SIZE; moved = true; }
    if (IsKeyDown(KEY_UP))    { editor.cursor.y -= SQUARE_SIZE; moved = true; }
    if (IsKeyDown(KEY_DOWN))  { editor.cursor.y += SQUARE_SIZE; moved = true; }
