...
typedef struct { int count; int keys[8]; } KeyList;
...
static const KeyName KEY_NAMES[] = {
    {"SPACE", KEY_SPACE}, {"ENTER", KEY_ENTER}, {"RETURN", KEY_ENTER}, {"ESCAPE", KEY_ESCAPE},
    {"TAB", KEY_TAB},
    {"UP", KEY_UP}, {"DOWN", KEY_DOWN}, {"LEFT", KEY_LEFT}, {"RIGHT", KEY_RIGHT},
    {"W", KEY_W}, {"A", KEY_A}, {"S", KEY_S}, {"D", KEY_D},
    {"ONE", KEY_ONE}, {"TWO", KEY_TWO}, {"THREE", KEY_THREE}, {"FOUR", KEY_FOUR}, {"FIVE", KEY_FIVE},
};
...
static void AddKey(InputAction a, int key)
{
    ...
    // prevent duplicates
    for (int i=0;i<kl->count;i++) if (kl->keys[i] == key) return;
...
static void LoadDefaults(void)
{
    ...
    AddKey(ACT_ACTIVATE, KEY_ENTER); AddKey(ACT_ACTIVATE, KEY_SPACE);
    AddKey(ACT_BACK, KEY_ESCAPE);
    AddKey(ACT_NAV_UP, KEY_UP); AddKey(ACT_NAV_UP, KEY_W);
    AddKey(ACT_NAV_DOWN, KEY_DOWN); AddKey(ACT_NAV_DOWN, KEY_S);
    AddKey(ACT_NAV_LEFT, KEY_LEFT); AddKey(ACT_NAV_LEFT, KEY_A);
    AddKey(ACT_NAV_RIGHT, KEY_RIGHT); AddKey(ACT_NAV_RIGHT, KEY_D);
    AddKey(ACT_LEFT, KEY_LEFT); AddKey(ACT_LEFT, KEY_A);
    AddKey(ACT_RIGHT, KEY_RIGHT); AddKey(ACT_RIGHT, KEY_D);
    AddKey(ACT_DOWN, KEY_DOWN); AddKey(ACT_DOWN, KEY_S);
    AddKey(ACT_JUMP, KEY_SPACE); AddKey(ACT_JUMP, KEY_W); AddKey(ACT_JUMP, KEY_UP);
}
...
static const char* ActionName(InputAction a)
{
    switch (a) {
        ...
        case ACT_NAV_UP: return "nav_up";
        case ACT_NAV_DOWN: return "nav_down";
        case ACT_NAV_LEFT: return "nav_left";
        case ACT_NAV_RIGHT: return "nav_right";
...
static InputAction ActionByName(const char* name)
{
    ...
    for (int a=0;a<ACT__COUNT;a++) if (strcmp(p, ActionName((InputAction)a))==0) return (InputAction)a;
    return ACT__COUNT;
}
...
static void TryLoadFile(const char* path)
{
    ...
    while (fgets(line, sizeof(line), f))
    {
        ...
        *eq = '\0';
        ...
        char* vals = eq+1;
        // strip end of key
        char* end = keyName + strlen(keyName); while (end>keyName && isspace((unsigned char)end[-1])) *--end='\0';
        ...
        while (tok) {
            ...
            char* te = tok + strlen(tok); while (te>tok && isspace((unsigned char)te[-1])) *--te='\0';
...
bool InputDown(InputAction a)
{
    ...
    KeyList* kl = &gActions[a];
    for (int i=0;i<kl->count;i++) if (IsKeyDown(kl->keys[i])) return true;
    return false;
}
...
bool InputPressed(InputAction a)
{
    if (a < 0 || a >= ACT__COUNT) return false;
    KeyList* kl = &gActions[a];
    for (int i=0;i<kl->count;i++) if (IsKeyPressed(kl->keys[i])) return true;
    return false;
}
